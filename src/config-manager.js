...
class ConfigManager {
  ...
  async configureEmail() {
    ...
    // Email addresses
    console.log('\n--- Email Addresses ---');
    const fromEmail = await this.question(`From Address [${config.email.config.from || `Codex-Code-Remote <${config.email.config.smtp.auth.user}>`}]: `);
    config.email.config.from = fromEmail || config.email.config.from || `Codex-Code-Remote <${config.email.config.smtp.auth.user}>`;
  ...
  async interactiveMenu() {
    console.log('\n🛠️  Codex-Code-Remote Configuration Manager\n');
