...
/**
 * Codex-Code-Remote Daemon Service
...
class ClaudeCodeRemoteDaemon {
    ...
    async start(detached = true) {
        try {
            ...
            if (this.isAlreadyRunning()) {
                console.log('❌ Codex-Code-Remote daemon is already running');
    ...
    async startDetached() {
        console.log('🚀 Starting Codex-Code-Remote daemon...');
        ...
        console.log(`✅ Codex-Code-Remote daemon started (PID: ${child.pid})`);
    ...
    async startForeground() {
        console.log('🚀 Codex-Code-Remote daemon starting...');
    ...
    async stop() {
        if (!this.isAlreadyRunning()) {
            console.log('❌ Codex-Code-Remote daemon is not running');
            return;
        }

        try {
            const pid = this.getPid();
            console.log(`🛑 Stopping Codex-Code-Remote daemon (PID: ${pid})...`);
            ...
            console.log('✅ Codex-Code-Remote daemon stopped');
        } catch (error) {
    ...
    async restart() {
        console.log('🔄 Restarting Codex-Code-Remote daemon...');
        await this.stop();
    ...
    showStatus() {
        ...
        console.log('📊 Codex-Code-Remote daemon status\n');
