...
class DesktopChannel extends NotificationChannel {
    ...
    _sendMacOS(title, message, sound) {
        try {
            ...
            try {
                const cmd = `terminal-notifier -title "${title}" -message "${message}" -sound "${sound}" -group "codex-code-remote"`;
    ...
    _sendWindows(title, message, sound) {
        try {
            ...
            [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("Codex-Code-Remote").Show($toast)
            `;
