...
/**
 * Smart Command Injector - Multiple methods to ensure commands reach Codex
 */
        ...
        return new Promise((resolve) => {
            ...
            this.copyToClipboard(command).then(() => {
                ...
                tell application "System Events"
                    set targetApps to {"Codex", "OpenAI Codex", "Claude", "Claude Code", "Terminal", "iTerm2", "iTerm"}
    ...
    async tryClipboardWithPersistentNotification(token, command) {
        return new Promise((resolve) => {
            this.copyToClipboard(command).then(() => {
                ...
                const notifications = [
                    { delay: 0, sound: 'Basso', message: '🚨 Email command copied! Please paste immediately to Codex (Cmd+V)' },
                    { delay: 3000, sound: 'Ping', message: '⏰ Reminder: Command still in clipboard, please paste and execute' },
                    { delay: 8000, sound: 'Purr', message: '💡 Final reminder: Press Cmd+V in Codex to paste command' }
                ];
...
echo "Copied to clipboard, please press Cmd+V in Codex to paste"
echo "${command}" | pbcopy
