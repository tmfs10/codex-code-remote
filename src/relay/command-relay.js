 ...
 * Manages email listening and command execution for Codex CLI
 */
...
class CommandRelayService extends EventEmitter {
    constructor(config) {
        ...
        this.codexAutomation = new ClaudeAutomation();
    ...
    async _executeCommand(commandItem) {
        ...
        try {
            ...
            if (!codexProcess || !codexProcess.available) {
            ...
            if (success) {
                ...
                throw new Error('Failed to send command to Codex');
            }
    ...
    async _findCodexProcess() {
        return new Promise((resolve) => {
            // Find Codex CLI related processes
            ...
            ps.on('close', (code) => {
                ...
                const codexProcesses = lines.filter(line => 
                    (line.includes('codex') || 
                     line.includes('@openai/codex') ||
                     (line.includes('OpenAI') && line.includes('codex')) ||
                     (line.includes('node') && line.includes('codex'))) && 
                    !line.includes('grep') &&
                ...
                if (codexProcesses.length > 0) {
                    // Parse process ID
                    const processLine = codexProcesses[0];
                    ...
                    this.logger.debug('No Codex process found, will try desktop automation');
                    resolve({ pid: null, available: true });
                }
            });

            ps.on('error', (error) => {
                this.logger.error('Error finding Codex process:', error.message);
    ...
    async _sendCommandToCodex(command, codexProcess, sessionId) {
        return new Promise(async (resolve) => {
            try {
                // Method 1: Codex dedicated automation (most direct and reliable)
                this.logger.info('Attempting to send command via Codex automation...');
                const codexSuccess = await this.codexAutomation.sendCommand(command, sessionId);
                
                if (codexSuccess) {
                    this.logger.info('Command sent and executed successfully via Codex automation');
                    resolve(true);
                    return;
                }
                ...
                this.logger.error('Error sending command to Codex:', error.message);
                resolve(false);
            }
        });
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            const script = `
                tell application "System Events"
                    ...
                    -- Try to find Codex, Terminal or other development tools
                    set targetApps to {"Codex", "OpenAI Codex", "Terminal", "iTerm2", "iTerm", "Visual Studio Code", "Code"}
        ...
        return new Promise((resolve) => {
            try {
                ...
                if (process.platform === 'darwin') {
                    ...
                    const script = `
                        display notification "Command: ${commandPreview.replace(/"/g, '\\"')}" with title "TaskPing - Email Command" subtitle "Click Terminal or Codex window, then paste command" sound name "default"
                    `;
