...
// Extract Claude/Codex-Code-Remote token from subject
function extractTokenFromSubject(subject = '') {
    const patterns = [
        /\[(Claude|Codex)-Code-Remote\s+#([A-Z0-9]+)\]/,
        /Re:\s*\[(Claude|Codex)-Code-Remote\s+#([A-Z0-9]+)\]/
    ];
...
function cleanEmailText(text = '') {
    ...
    for (const line of lines) {
        ...
        if (line.includes('-----Original Message-----') ||
            ...
            (line.includes('Claude-Code-Remote Notification System') || line.includes('Codex-Code-Remote Notification System')) ||
    ...
    for (const line of contentLines) {
        ...
        if (trimmedLine.includes('Claude-Code-Remote Notification System') ||
            trimmedLine.includes('Codex-Code-Remote Notification System') ||
...
async function injectCommandRemote(token, command) {
    ...
    try {
        ...
        const tmuxSessionName = session.tmuxSession || 'codex-taskping';
    ...
    return new Promise((resolve) => {
        ...
        pbcopy.on('close', (code) => {
            ...
            tell application "System Events"
                set claudeApps to {"Codex", "OpenAI Codex", "Claude", "Claude Code", "Terminal", "iTerm2", "iTerm"}
            ...
            exec(`osascript -e '${autoScript}'`, (error, stdout, stderr) => {
                ...
                switch(result) {
                    case 'terminal_typed':
                        ...
                        break;
                    case 'codex_pasted':
                        resolve({ success: true, method: 'Codex app paste' });
                        break;
    ...
    return new Promise((resolve) => {
        ...
        pbcopy.on('close', (code) => {
            ...
            const notificationScript = `
                display notification "🚨 Email command auto-copied! Please paste and execute in Codex immediately (Cmd+V)" with title "TaskPing Auto-Injection" subtitle "${shortCommand.replace(/"/g, '\\"')}" sound name "Basso"
            `;
