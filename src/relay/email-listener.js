...
class EmailListener extends EventEmitter {
    ...
    _isReplyEmail(email) {
        // Check if subject contains Claude/Codex-Code-Remote identifier
        const subject = email.subject || '';
        if (!subject.includes('[Claude-Code-Remote') && !subject.includes('[Codex-Code-Remote')) {
            return false;
        }
    ...
    _extractSessionId(email) {
        ...
        if (headers && headers.get('x-codex-code-remote-session-id')) {
            return headers.get('x-codex-code-remote-session-id');
        }
        ...
        const subject = email.subject || '';
        const tokenMatch = subject.match(/\[(Claude|Codex)-Code-Remote #([A-Z0-9]{6,8})\]/);
        if (tokenMatch) {
            const token = tokenMatch[2] || tokenMatch[1];
