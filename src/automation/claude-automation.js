/**
 * Codex CLI Dedicated Automation
 * Complete automation solution specifically designed for OpenAI Codex
 */
...
class ClaudeAutomation {
    constructor() {
        this.logger = new Logger('CodexAutomation');
    }

    /**
     * Fully automated command sending to Codex CLI
    ...
    async sendCommand(command, sessionId = '') {
        try {
            this.logger.info(`Sending command to Codex CLI: ${command.substring(0, 50)}...`);
            ...
            this.logger.error('Codex automation failed:', error.message);
            return false;
        }
    }
        ...
        return new Promise((resolve) => {
            const script = `
                tell application "System Events"
                    -- Define possible Codex application names
                    set claudeApps to {"Codex", "OpenAI Codex", "Claude", "Claude Code", "Claude Desktop", "Anthropic Claude"}
                    ...
                    -- First try to find Codex
                    set targetApp to null
                    set appName to ""
                    
                    -- Check Codex applications
                    ...
                    -- If Codex not found, check terminal applications
                    ...
                    if targetApp is not null then
                        ...
                        if appName contains "Codex" or appName contains "Claude" then
                            -- Codex/Claude specific operations
                            try
    ...
    /**
     * Specifically activate Codex CLI application
     */
    async activateCodex() {
        if (process.platform !== 'darwin') {
            return false;
        ...
        return new Promise((resolve) => {
            ...
            osascript.on('close', (code) => {
                if (code === 0 && output.startsWith('activated:')) {
                    this.logger.info('Codex activated successfully');
                    resolve(true);
                } else {
                    this.logger.warn('Could not activate Codex');
                    resolve(false);
                }
            });
