...
if (!config.channelAccessToken || !config.channelSecret) {
    logger.error('LINE_CHANNEL_ACCESS_TOKEN and LINE_CHANNEL_SECRET must be set in ~/.codex_code_remote_env');
    process.exit(1);
}

if (!config.userId && !config.groupId) {
    logger.error('Either LINE_USER_ID or LINE_GROUP_ID must be set in ~/.codex_code_remote_env');
    process.exit(1);
}
