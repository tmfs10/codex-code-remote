...
if (!config.botToken) {
    logger.error('TELEGRAM_BOT_TOKEN must be set in ~/.codex_code_remote_env');
    process.exit(1);
}

if (!config.chatId && !config.groupId) {
    logger.error('Either TELEGRAM_CHAT_ID or TELEGRAM_GROUP_ID must be set in ~/.codex_code_remote_env');
    process.exit(1);
}
