...
/**
 * Claude-Code-Remote - Codex Smart Notification System
 * Main entry point for the CLI tool
 */

// Load environment variables from Codex-Code-Remote directory
...
class ClaudeCodeRemoteCLI {
        ...
        try {
            ...
            switch (command) {
                ...
                case 'test-simple':
                    ...
                    break;
                case 'test-codex':
                    await this.handleTestCodex(args.slice(1));
                    break;
                case 'test-claude':
                    await this.handleTestCodex(args.slice(1));
                    break;
    ...
    async handleStatus(args) {
        ...
        console.log('Codex-Code-Remote Status\n');
    ...
    async startRelay(args) {
        try {
            ...
            relayService.on('started', () => {
                ...
                console.log('💡 You can now remotely execute Codex commands by replying to emails');
                console.log('');
    ...
    async handleSetupEmail(args) {
        ...
        try {
            console.log('🚀 Codex-Code-Remote Email Quick Setup Wizard\n');
    ...
    async handleTestPaste(args) {
        ...
        console.log('\n⚠️  Please ensure Codex or Terminal window is open and active');
    ...
    async handleTestSimple(args) {
        ...
        try {
            ...
            if (success) {
                ...
                console.log('4. Manually paste to Codex (if auto-paste didn\'t work)');
    ...
    async handleMonitor(args) {
        ...
        monitor.on('taskCompleted', async (event) => {
            const conversation = event.conversation || {};
            await this.notifier.notify('completed', {
                userQuestion: conversation.userQuestion,
                claudeResponse: conversation.claudeResponse,
                tmuxSession: event.sessionName,
                triggerText: event.triggerText
            });
        });

        monitor.on('waitingForInput', async (event) => {
            const conversation = event.conversation || {};
            await this.notifier.notify('waiting', {
                userQuestion: conversation.userQuestion,
                claudeResponse: conversation.claudeResponse,
                tmuxSession: event.sessionName,
                triggerText: event.triggerText
        ...
        console.log('📡 Starting tmux monitor for Codex...');
        monitor.start();
    ...
    async handleTestCodex(args) {
        ...
        console.log('🤖 Testing Codex specialized automation');
        console.log(`📝 Test command: ${testCommand}`);
        console.log('\n⚠️  Please ensure:');
        console.log('   1. Codex CLI is open');
        ...
        try {
            ...
            if (success) {
                ...
                console.log('💡 Command should have been automatically input to Codex and started execution');
                console.log('🔍 Please check Codex/Terminal window to see if command was received');
            } else {
                console.log('❌ Automation test failed');
                console.log('💡 Possible reasons:');
                console.log('   • Codex or terminal application not found');
                console.log('   • Insufficient permissions');
                console.log('   • Application not responding');
                console.log('\n🔧 Suggestions:');
                console.log('   1. Run "claude-remote setup-permissions" to check permissions');
                console.log('   2. Ensure Codex is running in foreground');
                console.log('   3. Try manually clicking input box in Codex/Terminal first');
            }
        } catch (error) {
            console.error('❌ Error occurred during test:', error.message);
        }
...
Commands:
  ...
  install                 Check Codex CLI setup and initialize configuration
...
Examples:
  ...
  claude-remote monitor                   # Auto-detect Codex completion in tmux
  claude-remote test-codex                # Test full automation (recommended)
