...
/**
 * Codex Hook Notification Script
 * Called by Codex CLI workflows to send notifications
 */
...
async function sendHookNotification() {
    try {
        console.log('🔔 Codex Hook: Sending notifications...');
        ...
        let tmuxSession = process.env.TMUX_SESSION || 'codex-real';
        try {
        ...
        const notification = {
            ...
            title: `Codex ${notificationType === 'completed' ? 'Task Completed' : 'Waiting for Input'}`,
            message: `Codex has ${notificationType === 'completed' ? 'completed a task' : 'is waiting for input'}`,
            project: projectName
